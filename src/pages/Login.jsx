...
function Login() {
  ...
  return (
    ...
    <div className="loginRight mx-auto text-white">
      <h1 className='text-white text-6xl font-bold mt-30 mb-15 text-[rgb(231, 233, 234)]'>X but not really..</h1>
      ...
      <label className='font-medium' htmlFor="email">Email</label>
        ...
        {errors.email && <span className='text-red-500 text-xs '>Email is required</span>}</div>
        <div className="password flex flex-col gap-1"> <label className='font-medium' htmlFor="password">Password</label>
         ...
         {errors.password && <span className='text-red-500 text-xs '>Password is required</span>}</div>
      ...
      <button className='border border-gray-500 w-50 rounded-2xl p-2 mx-auto cursor-pointer' onClick={()=>{nav('/signup')}} >Sign in</button></div></div>
    </div>
    <Footer/>
    </div>
  )
